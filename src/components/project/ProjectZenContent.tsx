    ...
    const renderModule = (m: any) => {
        ...
        return (
            <div key={m.id} className="mb-10 animate-in fade-in slide-in-from-bottom-2 duration-700">
                ...
                {/* Review / Full Mode Elements */}
                {(mode === 'REVIEW' || mode === 'FULL') && (
                 ...
                 <section className="pt-20 border-t-4 border-slate-100">
                    ...
                    {project.works.map((w: any) => (
                        <div key={w.id} className="mb-20">
                             <h3 className="text-2xl font-black text-slate-800 mb-6">{w.title}</h3>
                             {w.modules?.map(renderModule)}
                             {/* Recursive tasks/activities omitted for brevity or handled similar to above */}
                        </div>
                    ))}
                 </section>
            )}
